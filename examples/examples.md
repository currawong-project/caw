...
Label      | Purpose
-----------|-----------------------------------------------------------------------
`base_dir` | Base directory for all project files. '$' = `<base_dir>/<pgm_name>`
`io_dict`  | Runtime cfg. file. e.g. `"~/src/caw/src/caw/cfg/io.cfg"`
`proc_dict`| Processor class dictionary.   e.g. `"~/src/caw/src/libcw/src/flow/rsrc/proc_dict.cfg"`
`udp_dict` | User Defined Processor dictionary. e.g. `"~/src/caw/src/libcw/src/flow/rsrc/udp_dict.cfg"`
`tracer`   | Trace configuration: `{  trace_cnt:1024, msg_cnt: 1024000, enable_fl:false, activate_fl:false, out_fname:"tracer" }`
           | `trace_cnt` = Maximum count of traces.
           | `msg_cnt`   = Maximum count of log messages generated by all traces.
           | `enable_fl` = Enable tracing by creating the global tracer object. Set to 'false' for release build.
           | `activate_fl` = Start/Stop recording of traces.
           | `out_fname` = Tracer output file name written on program exit. File format:`"seconds,nanosecs,app_trace_id,event_id,user0,user1"`
`log`      | Log configuration  `{ flags:[ date_time, file_out, console, overwrite_file ], level:debug, log_filename:"log.txt", queue_blk_cnt:16, queue_blk_byte_cnt:4096 }`
           | flags 
           | level
           | log_filename
           | queue_blk_cnt
